...
- In-page find: press `Ctrl+F` (macOS `Cmd+F`) or click the magnifier button in the toolbar to open a find bar; all matches are highlighted, with match count, next/previous navigation, `Enter` / `Shift+Enter` shortcuts and `Esc` to close.
  <br>页面内查找：按 `Ctrl+F`（macOS 为 `Cmd+F`）或点击工具栏放大镜按钮唤出查找栏，高亮全部匹配并显示匹配总数，支持上一个/下一个切换、`Enter` / `Shift+Enter` 快捷键与 `Esc` 关闭。
- The find shortcut is intercepted by the browser engine, so it works whether the focus is inside the web page or on the panel.
